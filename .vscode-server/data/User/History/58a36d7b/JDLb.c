               ...
               thread_func *function, void *aux) 
{
  ...
  /* Add to run queue. */
  thread_unblock (t);
  thread_test_preemption();  //  ------------------------ Project 1 Priority Scheduling -----------------------------
...
// -------------------------------------- Project1 Priority Scheduling ---------------------------------------------
// -------------------------------------- Project1 Advanced Scheduler ----------------------------------------------
void mlpqs_calculate_priority (struct thread *t)
{
  if(t == idle_thread)
  {
    return;
...
void mlfqs_calculate_recent_cpu (struct thread *t)
{
  ...
  t->recent_cpu = add_mixed(mult_fp(div_fp(mult_mexed(load_avg, 2), add_mixed(mult_mixed(load_avg, 2), 1)), t-> recent_cpu), t-> nice);
}
